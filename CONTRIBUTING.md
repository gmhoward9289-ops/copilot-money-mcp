...
- **`src/tools/tools.ts`** — All 33 base tools (14 read + 19 write) as async methods in `CopilotMoneyTools`. Read tool schemas in `createToolSchemas()`, write tool schemas in `createWriteToolSchemas()`.
