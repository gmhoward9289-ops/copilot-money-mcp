...
- **`src/tools/tools.ts`** - All 32 base tools (14 read + 18 write) are implemented here as async methods in the `CopilotMoneyTools` class. Read schemas in `createToolSchemas()`, write schemas in `createWriteToolSchemas()` — both are pure projections of the registry. The 17 live-mode tools live in `src/tools/live/*.ts`.
