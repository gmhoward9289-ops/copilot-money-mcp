...
export class CopilotMoneyTools {
  ...
  async reviewTransactions(args: {
    ...
    // Bounded concurrency via the shared pool (see runBoundedPool): never more
    // than BULK_WRITE_CONCURRENCY in flight, first error stops new writes and
    // in-flight ones settle, and the success count stays accurate so partial
    // failure is reportable.
    let reviewed_count = 0;
    const failures = await CopilotMoneyTools.runBoundedPool(
      entries,
      BULK_WRITE_CONCURRENCY,
      true,
      async (entry) => {
